<script lang="ts">
    ...
    let progress = $state({ step: 'starting', percent: 0, install_status: 'pending' });
        ...
        return watchInstallProgress(
            ...
            (payload) => {
                progress = {
                    ...
                    install_status: payload.install_status ?? progress.install_status ?? 'pending',
                };

                if (payload.step === 'ready' || payload.install_status === 'ready') {
                    router.visit('/');
                }
            },
        );
...
<div class="flex min-h-dvh flex-col items-center justify-center gap-6 p-8">
    <div class="text-center">
        ...
        <p class="text-base-content/70 mt-2 max-w-md">
            Setting up the bundled American Standard Version translation. This runs once on first launch.
        </p>
    </div>

    <div class="w-full max-w-md">
        <progress class="progress progress-primary w-full" value={progress.percent} max="100"></progress>
        <p class="text-base-content/60 mt-2 text-center text-sm capitalize">{progress.step.replaceAll('_', ' ')}</p>
    </div>
</div>
